pgbart_train = function(
   x.train, y.train,model,
...
)
{
   ...
   if(typeof(model) != "character") stop("model must be a character")
   tmp = try(write.table(list(), model))
   if ('try-error' %in% class(tmp)) stop("can't create model, check the path")
   ...
   metaData = c(rgy[2] - rgy[1], rgy[1], ncpost, binary, binaryOffset)
   write.table(x = metaData, file = model, row.names = FALSE, col.names = FALSE)
   ...
   cres = .C('mbart',as.integer(nrow(x.train)), as.integer(ncol(x.train)), as.integer(nrow(x.test)),
                   ...
                   as.double(x.test),
                   as.character(model), as.integer(ncskip*keepevery),
