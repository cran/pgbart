	   ...
	   double *sdraw, double *trdraw, double *tedraw, int *vcdraw)
{
   ...
   const char* model_path = *imodelpath;

   Rprintf("model_path = %s\n", model_path);
...
void mpredict(int* inrow, int* idim, double* idata,
              char** imodelpath,
              double* tedraw){
   ...
   double* fit = new double[nrow + 1];
   int tecnt = 0;


   for(int k = 1; k <= ncpost; k++){
      for(int i = 1; i<trees.size(); i++)
         trees[i] = new BartNode;
      memset(fit, 0, sizeof(double) * (nrow + 1));
      read_itr(fp, trees, ntree);
      for(int i = 1; i <= nrow; i++){
         for(int j = 1; j <= ntree; j++){
            fit[i] += predict(trees[j], X[i]);
         }
         tedraw[tecnt++] = fit[i];
      }
      clear_itr(trees, ntree);
   }
